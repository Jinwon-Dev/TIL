...
1. 교착 상태가 발생한 것을 운영자(operator)에게 통지해, 운영자가 수작업으로 처리하게 한다.
2. 시스템이 자동으로 교착 상태로부터 회복하게 한다.

</br>

> **교착 상태를 깨뜨리는 데는 두 가지 방법이 있다.**

1. **순환 대기를 깨뜨리기 위해 단순히 한 개 이상의 스레드를 중지**시킨다.
2. **교착 상태에 있는 하나 이상의 스레드들로부터 자원을 선점**한다.

</br>

### 프로세스와 스레드의 종료

→ ***프로세스 또는 스레드를 중지시킴으로써 교착 상태를 제거하기 위해 두 방법중 하나를 사용한다.***

- 두 방법 모두, **종료된 프로세스에게 할당되었던 모든 자원을 시스템이 회수**한다.

</br>

1. **교착 상태 프로세스를 모두 중지** : 확실하게 교착 상태의 사이클을 깨뜨리지만, 비용이 크다.
    - 이들 프로세스가 오랫동안 연산했을 가능성이 있으며, 이들 부분 계산의 결과들을 반드시 폐기해야 하며, 다시 계산해야 하기 때문이다.

</br>

2. **교착 상태가 제거될 때까지 한 프로세스씩 중지** : 각 프로세스가 중지될 때마다 교착 상태 탐지 알고리즘을 호출해야 한다.
    - 상당한 오버헤드를 유발한다.
...
- 만약 프로세스가 파일을 갱신하는 도중이었다면, 중간에 종료시킬 경우 파일은 부정확한 상태가 된다.

</br>

> **부분 종료 방식을 사용한다면, 교착 상태인 프로세스들의 집합 중에서 교착 상태를 깨뜨리기 위해 어느 프로세스를 중지해야 할지 결정해야 한다.**
...
→ ***자원 선점을 이용해 교착 상태를 제거하려면, 교착 상태가 깨질 때까지 프로세스로부터 자원을 계속 선점해 이들을 다른 프로세스에 주어야 한다.***

- 선점이 필요하면, 다음의 세 가지 사항들을 고려해야 한다.

</br>

1. **희생자 선택(selection of a victim)** : **어느 자원과 어느 프로세스들이 선점될 것인가?**
    - 프로세스 종료에서와 같이, 비용을 최소화하기 위해 선점의 순서를 결정해야 한다.

</br>

2. **후퇴(rollback)** : **프로세스로부터 자원을 선점하려면, 그 프로세스를 어떻게 해야 하는가?**
    - 확실한 것은 이 프로세스를 계속 정상적으로 실행할 수 없다는 점이다.
    - 일반적으로 안전 상태가 어떤 것인지를 결정하기 어렵기 때문에, 가장 단순한 해결법은 완전히 후퇴시키는 것이다.
    - 하지만, 이 방식은 시스템이 실행하는 모든 프로세스의 상태에 많은 정보를 유지할 것을 필요로 한다.

</br>

3. **기아 상태(starvation)** : **기아 상태가 발생하지 않는 것을 어떻게 보장할 것인가?**
    - 즉, 자원들이 동일한 프로세스로부터 항상 선점되지 않는다는 것을 어떻게 보장하는가?

</br>

→ **희생자의 선택이 주로 비용 요인에 근거한 시스템에서는 동일한 프로세스가 항상 희생자로 선택될 수도 있다!**