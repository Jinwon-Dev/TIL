...
→ ***IA-32 시스템의 메모리 관리는 세그먼테이션과 페이징의 두 부분으로 나눌 수 있으면 아래와 같이 동작한다.***

<img width="537" alt="image" src="https://user-images.githubusercontent.com/106216912/221360116-780b779d-331d-4e9c-89c1-a64f7252d8cb.png">
...
→ **IA-32 구조는 하나의 세그먼트가 최대 4GB의 크기를 가질 수 있으며, 한 프로세스당 16KB개의 세그먼트를 가질 수 있다.**

- 각 프로세스의 주소 공간은 두 개의 파티션으로 나누어진다.
    - 첫 번째 파티션에 대한 정보는 **지역 디스크립터 테이블(LDT)** 에 유지된다.
    - 두 번째 파티션에 대한 정보는 **전역 디스크립터 테이블(GDT)** 에 저장된다.

</br>

<img width="244" alt="image" src="https://user-images.githubusercontent.com/106216912/221360150-ea8b88bc-143a-43b7-8baf-e43c6d07b65d.png">

- `s` : 세그먼트 번호
- `g` : 세그먼트가 GDT인지 LDT인지를 나타냄
- `p` : 보호와 관련된 정보를 나타냄

</br>

> **IA-32에서 선형 주소의 길이는 32비트이며, 다음과 같이 구성된다.**

<img width="393" alt="image" src="https://user-images.githubusercontent.com/106216912/221360174-6d75a3d9-8530-40dc-a947-8d627733cf58.png">

- 세그먼트 레지스터가 LDT나 GDT 내의 적절한 항목을 가리킨다.
    - 일단 찾고자 하는 세그먼트에 대한 기준과 상한 정보를 가져와서 실제 선형 주소를 만들어내는 데 사용한다.
    - 주소가 한계치를 넘어가면 메모리 오류가 발생되어 운영체제에 트랩을 발생시킨다.
    - 주소가 타당하면 오프셋 값이 기준 값에 더해져서 32비트 선형 주소를 생성한다.

</br>

> **IA-32 페이징**

→ ***IA-32는 4KB 크기의 페이지를 사용할 때 2단계 페이징 기법을 사용한다.***

<img width="336" alt="image" src="https://user-images.githubusercontent.com/106216912/221360206-7bf7cb35-b945-4e54-8cd6-e4c49f2c8dd0.png">

</br>

> **IA-32의 주소 변환**

- 10개의 상위 비트는 IA-32 구조에서 **페이지 디렉터리**라고 부르는 최상위 페이지 테이블의 엔트리를 가리킨다.
    - 페이지 디랙터리 엔트리는 다음 10개에 의해 인덱스 되는 하나의 하위 페이지 테이블을 가리킨다.
    - 마지막 하위 12비트는 페이지 테이블이 가리키고 있는 4KB 페이지의 오프셋을 나타낸다.

</br>

> **소프트웨어 개발자들은 IA-32 구조의 4GB 메모리 제한이 문제가 된다고 인식하여, 큰 물리 주소 공간에 접근할 수 있게 하는 페이지 주소 확장(PAE)를 채택하였다.**

<img width="428" alt="image" src="https://user-images.githubusercontent.com/106216912/221360239-73890d17-6fe3-4472-97f8-8ca33efcd013.png">

- PAE를 지원함으로써 2단계 기법에서 3단계 기법으로 변경되었다.
