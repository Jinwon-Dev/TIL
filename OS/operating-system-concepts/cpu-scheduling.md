...
→ ***특정 시스템을 위한 CPU 스케줄링 알고리즘은 어떻게 선택할까?***

- 문제는 알고리즘을 선택하는 데 사용할 기준을 정의하는 것이다.
    - CPU 이용률, 응답 시간, 처리량 등등

</br>

→ 선택 기준이 정의되면, 고려 중인 여러 가지 알고리즘들을 평가하기를 원한다.

</br>

### 결정론적 모델링

→ ***평가 방법의 한 중요한 부류로 분석적 평가가 있다.***

- **분석적 평가** : 주어진 작업 부하에 대한 알고리즘의 성능을 평가하는 공식이나 값을 생성하기 위해 주어진 알고리즘과 시스템 작업 부하를 이용한다.

</br>

> **분석적 평가의 한 가지 유형은 결정론적 모델링이다.**

→ **사전에 정의된 특정한 작업 부하를 받아들여, 그 작업 부하에 대한 각 알고리즘의 성능을 정의한다.**

- 단순하고 빠르고, 알고리즘들을 비교할 수 있도록 정확한 값을 제공한다.
- 단점 : **입력으로 정확한 숫자를 요구하며, 응답 또한 이들의 경우에만 적용**된다.
...
- 각 서버는 대기 프로세스들의 큐를 가지고 있다.
    - 도착률과 서비스율을 알기 때문에, 우리는 이용률, 평균 큐 길이, 평균대기 시간 등을 계산할 수 있다.
...
<img width="83" alt="image" src="https://user-images.githubusercontent.com/106216912/211486509-6536f81f-ae5c-44e1-8704-e44a08e543b5.png">

- 순서대로 평균 큐 길이, 새로운 프로세스들이 큐에 도착하는 평균 도착률, 큐에서의 평균대기 시간이다.

</br>

- 큐잉 분석은 스케줄링 알고리즘을 비교하는 데 유용하게 사용할 수 있으나, **한계**가 있다.
    - 분석할 수 있는 알고리즘과 분포의 부류가 상당히 제한되어 있다.
    - 정확하지 않을 수 있는, 다수의 독립적 가정을 하는 것이 일반적으로 필요하다.
...
<img width="433" alt="image" src="https://user-images.githubusercontent.com/106216912/211486697-1b671bb1-ad7d-4abe-bef3-1696a075a4b0.png">

- 모의실험을 구동하기 위한 자료들은 여러 가지 방법으로 생성할 수 있다.
    - 난수 발생기
    - 추적 파일의 사용

</br>

- 모의 실험은 컴퓨터를 오랜 시간 사용하기 때문에, **매우 큰 비용**이 소요될 수 있다.
    - **대량의 저장 공간과 사용 시간**을 필요로 한다.
...
- 회귀 테스트를 통해 변경 사항이 더 나빠지거나, 버그가 발생하지 않았는지 확인할 수 있다.
- **단점** :  알고리즘이 사용되는 환경은 변한다.
    - 비용이 많이 든다.