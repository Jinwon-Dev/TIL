...
→ ***UNIX처럼 만일 장치 이름공간이 정규의 파일 시스템 이름공간과 결합하여 있으면, 통상의 파일 시스템 이름 서비스가 자동으로 제공된다.***

- 만일 파일 시스템이 모든 파일 이름에 대해 소유와 접근 제어를 제공하면, 장치도 소유와 접근 제어를 갖게 된다.
    - 파일이 장치에 저장되므로 이러한 인터페이스는 입출력 시스템에 대한 접근을 두 수준에서 제공한다.

</br>

> **UNIX는 장치 이름을 보통의 파일 이름처럼 표시한다.**
> 
- UNIX 경로명은 경로상의 어떤 부분도 장치 이름을 표면적으로 구별하여 나타내지 않는다.
    - 경로 이름의 접두어를 특정 장치 이름과 연관시키는 마운트 테이블을 갖고 있다.
    - 경로 이름을 해결하기 위해, UNIX는 **마운트 테이블**의 이름 중 접두어 부분이 경로명과 일치하는 가장 긴 이름을 찾는다.
...
> 
<img width="574" alt="image" src="https://user-images.githubusercontent.com/106216912/224379399-84873968-ef68-40ac-b2a1-2907b95a712f.png">

1. 한 프로세스가 앞서 오픈된 파일의 파일 디스크립터에 봉쇄형 `read` 시스템 콜을 요청한다.
2. 커널 안에 있는 시스템 콜 코드는 그 인자를 검사한다.
    1. `read` 의 경우에, 자료가 이미 버퍼 캐시에 들어있으면 그 자료를 반환하고 입출력 요구는 끝난다.
3. 없으면 실제의 `read` 가 필요하다.
    1. 이 때, 그 프로세스는 실행 큐에서 대기 큐로 옮겨지고 입출력 요구가 스케줄 된다.
4. 장치 드라이버는 입력될 자료를 위해 커널 버퍼를 확보하고, 입출력을 스케줄 한다.
5. 장치 컨트롤러가 장치를 작동하고, 데이터가 입력된다.
6. 장치 드라이버는 상태를 알아보거나 데이터를 얻기 위해 폴링할 수도 있고, DMA 방식으로 작업 처리를 해놓았을 수도 있다.
...
→ ***UNIX 시스템 V는 STREAMS라고 불리는 흥미로운 기법을 가지고 있다.***

<img width="384" alt="image" src="https://user-images.githubusercontent.com/106216912/224380747-07568917-1a7a-4ee2-9d45-a8f5cbfc1d18.png">

- 그것은 응용 프로그램이 동적으로 드라이버 코드의 파이프라인을 조립할 수 있게 한다.
    - 스트림은 디바이스 드라이버와 사용자 레벨 프로세스 사이의 완전 양방향 연결을 말한다.
    - 스트림은 사용자 프로세스와 상호 연동하는 **스트림 헤드**와 디바이스를 제어하는 **드라이버 엔드**, 그 둘 사이에 존재하는 0개 이상의 **스트림 모듈**로 구성되어 있다.

</br>

> **모듈은 스트림 처리 기능을 제공하며, `ioctl()` 시스템 콜을 사용하여 스트림에 푸시된다.**
> 
- 사용자 프로세스는 `write()` 또는 `putmsg()` 시스템 콜을 사용함으로써 디바이스에 데이터를 쓸 수 있다.
    - STREAMS 입출력은 사용자 프로세스가 스트림 헤드와 통신할 때를 제외하고 비동기이다.

</br>

→ STREAMS를 사용함으로써 얻어지는 이득은, **디바이스 드라이브와 네트워크 프로토콜을 작성할 때 모듈식이며, 점진적인 접근을 위한 프레임워크를 제공한다는 것이다!**
...
> **네트워크의 트래픽 또한, 문맥 교환을 매우 많이 야기할 수 있다.**
> 
<img width="532" alt="image" src="https://user-images.githubusercontent.com/106216912/224382352-c003e4c1-887c-42ff-acf6-78c9e10ee088.png">

- 일부 시스템들은 터미널 입출력을 전담하는 **프론트 엔드 처리기**를 사용하여 CPU로부터 인터럽트 부담을 덜고 있다.
    - ex) **터미널 집중기**는 수백 개의 원격 터미널에서 들어오는 정보를 멀티플렉스 할 수 있다.
    - **입출력 채널**은 전용의 특수 목적 CPU로, 메인 프레임과 대형 시스템에서 이러한 목적으로 사용된다.

</br>

> **입출력의 효율을 높이기 위해서는 다음과 같은 여러 가지 원칙들을 적용할 수 있다.**
> 
- 문맥 교환의 빈도를 줄인다.
- 메모리에서 장치와 응용 프로그램 사이에 데이터가 복사되는 횟수를 줄인다.
- 인터럽트 빈도를 줄인다.
- DMA나 채널 등을 사용해서 CPU의 입출력 부담을 줄이고, 입출력과 주 연산이 될 수록 많이 중첩되도록 도모한다.
