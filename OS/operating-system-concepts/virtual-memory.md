...
- 또한, 페이지 폴트가 발생할 때마다 프레임을 할당할 수도 있다.
    - 사용자 프로세스는 어떤 가용 프레임이든 할당받아야 한다.
...
- **프로세스에 할당되는 프레임 수가 줄어들면 페이지 폴트율은 증가하고, 프로세스 실행은 늦어진다.**
- **또한, 명령어 수행이 완료되기 전에 페이지 폴트가 발생하면 그 명령어를 재실행해야 한다.**
...
- 반면, 최대 수는 **사용 가능한 물리 메모리 양**에 의해 정의된다.

</br>

### 할당 알고리즘

→ **`n` 개의 프로세스에 `m` 개의 프레임을 분할하는 가장 쉬운 방법은 모두에 같은 몫 `m/n` 프레임씩 주는 것이다.**
...
- **균등 할당(equal allocation)**
    - **모든 프로세스에 똑같은 프레임을 할당하고, 남은 프레임은 가용 프레임 버퍼 저장소로 사용한다.**
    - ex) 93개의 프레임과 5개의 프로세스가 있으면, 각 프로세스는 18개씩의 프레임을 받는다.
    - 나머지 3개는 가용 프레임 버퍼 저장소로 사용한다.

</br>

- **비례 할당(proportional allocation)**
    - **가용 메모리를 각 프로세스의 크기 비율에 맞추어 할당한다.**
    - ex) 62프레임을 10페이지 크기의 프로세스와 127페이지 크기의 프로세스에 각 4프레임과 57프레임을 할당

</br>

> **두 방법 모두 다중 프로그래밍 정도에 따라 할당되는 양이 달라진다.**

- 다중 프로그래밍 정도가 높아지면 각 프로세스는 다 덜 받게 되고, 다중 프로그래밍 정도가 낮아지면 더 넉넉히 받게 된다.
    - 하지만, 균등 할당이나 비례 할당은 모두 우선순위는 신경쓰지 않고 있다는 사실에 유의해야 한다!

</br>

### 전역 대 지역 할당

→ ***다수의 프로세스가 프레임 할당을 위해 경쟁하는 환경에서 페이지 교체 알고리즘은 전역 교체, 지역 교체로 나눌 수 있다.***

- **전역 교체**
    - **프로세스가 교체할 프레임을 다른 프로세스에 속한 프레임을 포함한 모든 프레임을 대상으로 찾는다.**
    - 한 프로세스에 할당된 프레임의 수는 바뀐다.
    
</br>

- **지역 교체**
    - **각 프로세스가 자기에게 할당된 프레임 중에서만 교체될 희생자를 찾는다.**
...
→ ***프로세스의 메모리에 있는 페이지 집합이 해당 프로세스뿐만 아니라, 다른 프로세스의 페이징 동작에서 영향을 받는다.***

- 지역 교체 방법에서는 이러한 현상이 발생하지 않는다.

</br>

> **전역 페이지 교체 정책을 구현하는 데 사용할 수 있는 전략**

<img width="438" alt="image" src="https://user-images.githubusercontent.com/106216912/221391198-f820d1f2-eb84-4126-97bb-44a0e4d41ff9.png">

- **가용 메모리 양을 최소 임계값 이상으로 유지하는 전략**이다.
    - 이 임계값 아래로 떨어지면 시스템의 모든 프로세스에서 페이지를 회수하기 시작하는 커널 루틴이 촉발된다.
        - 이러한 커널 루틴은 **리퍼(reaper)** 로 알려져 있으며, 모든 페이지 교체 알고리즘도 적용할 수 있다.
    - 사용 가능한 메모리 양이 최대 임계값에 도달하면 리퍼 루틴이 일시 중지되고, 사용 가능한 메모리가 다시 최소  임계값 아래로 떨어지면 다시 시작된다.

</br>

> **일반적으로 커널 리퍼 루틴은 LRU 근사 형태의 알고리즘을 사용한다.**

- 하지만, 리퍼 루틴이 가용 프레임 리스트를 최소 임계값 이상으로 유지할 수 없는 경우?
    - 운영체제는 2차 기회 알고리즘을 중단하고 순수한 FIFO를 사용할 것이다.
    - 더 극단적인 Linux의 예는, 사용 가능한 메모리의 양이 매우 낮은 수준으로 떨어지면 **메모리-부족(OOM) 킬러**라고 하는 루틴이 종료할 프로세스를 선택해서 메모리를 회수한다.

</br>

### 비균등 메모리 접근

→ ***여러 개의 CPU를 가진 비균등 메모리 접근(NUMA) 시스템에서는 모든 메인 메모리는 동등하게 접근되지 않는다.***

<img width="243" alt="image" src="https://user-images.githubusercontent.com/106216912/221391222-2789bd34-5c39-4d13-80ae-0862dd184f9d.png">
...
- CPU는 다른 CPU의 로컬 메모리보다 자신의 로컬 메모리에 빠르게 액세스 할 수 있다.
    - NUMA 시스템은 메인 메모리에 대한 모든 액세스가 동일하게 취급되는 시스템 보다 느리다.
...
- 프로세스가 페이지 폴트를 일으키면 NUMA 인식 가상 메모리 시스템은 프로세스가 실행 중인 CPU에 최대한 가까운 프레임을 할당한다.
...
→ ***프로세스에 충분한 프레임이 없는 경우, 즉 작업 집합의 페이지를 지원하는 데 필요한 최소 프레임도 없는 경우는 어떤 일이 발생할까?***

- 프로세스는 곧바로 페이지 폴트를 일으킬 것이다.
- 페이지 교체가 필요하지만, 이미 활발하게 사용되는 페이지들만으로 이루어져 있으므로, 어떤 페이지가 교체되든 바로 다시 필요해진다.
    - 결과적으로, 바로바로 반복해서 페이지 폴트가 발생하며 교체된 페이지는 다시 얼마 지나지 않아 읽어올 필요가 생긴다.

</br>

→ 이러한 과도한 페이징 작업을 **스래싱(thrashing)** 이라고 부른다!

- **어떤 프로세스가 실제 실행보다 더 많은 시간을 페이징에 사용하고 있는 현상**
    - **심각한 성능 문제**를 야기한다.
...
- 만약 **CPU 이용률이 너무 낮아지면, 새로운 프로세스를 시스템에 더 추가해서 다중 프로그래밍의 정도를 높인다.**
    - 이 때, 전역 페이지 교체 알고리즘을 사용해서 어떤 프로세스의 페이지인지에 대한 고려 없이 교체를 수행한다.

</br>

- 어떤 프로세스가 새로운 실행 단계로 진입하여 더 많은 프레임이 필요하다고 가정한다.
    - 페이지 폴트가 발생하면서, 다른 프로세스로부터 프레임들을 가져오게 될 것이다.
    - 그런데, 교체된 페이지들이 해당 프로세스에서 필요로 하는 것이었다면 또 페이지 폴트를 발생시키고 또 다른 프로세스에서 프레임을 가져온다.
    - 이러한 프로세스들이 페이지 스왑인, 스왑아웃을 위해 페이징 장치를 사용해야 한다.
...
- **CPU 스케줄러는 이용률이 떨어지는 것을 보고, 이용률을 높이기 위해 새로운 프로세스를 추가해서 다중 프로그래밍의 정도를 더 높인다.**
    - **새로 시작하는 프로세스는 실행 중인 프로세스들로부터 프레임을 가져오고자 하며, 더 많은 페이지 폴트와 더 긴 페이징 장치 대기 시간을 야기**한다.

</br>

<img width="431" alt="image" src="https://user-images.githubusercontent.com/106216912/221392782-bd253473-ee3d-41f5-bcef-78448d882217.png">

→ **결과적으로 CPU 이용률은 더욱 떨어지고, CPU 스케줄러는 다중 프로그래밍 정도를 더욱 높인다!**

- 결국 **스래싱이 일어나게 되고, 시스템의 처리율은 대단히 낮아지고 페이지 폴트는 상당히 늘어난다.**
    - 실질 메모리 접근 시간은 증가하고 프로세스들은 페이징하는 데 시간을 다 소비하게 되어 아무런 일도 할 수 없다.

</br>

> **지역 교체 알고리즘(우선순위 교체 알고리즘)을 사용하여 스래싱의 영향을 제한할 수 있다.**

→ ***하나의 프로세스가 스래싱을 시작하면 다른 프로세스의 프레임을 빼앗아 후자가 스래싱을 유발하도록 할 수 없다.***

- 스래싱 현상을 방지하기 위해서는 **각 프로세스가 필요로 하는 최소한의 프레임 개수를 보장해야 한다!**
    - 하지만 어떻게 각 프로세스가 필요로 하는 최소한의 프레임 수를 알 수 있을까?

</br>

→ **프로세스가 실제로 사용하고 있는 프레임의 수가 몇 개인가를 알아보는 방법**이 있다.

- 이 방법은 프로세스 실행의 **지역성 모델(locality model)** 을 기반으로 한다.

</br>

> **지역성 모델이란 프로세스가 실행될 때는 항상 어떤 특정한 지역에서만 메모리를 집중적으로 참조함을 말한다.**

<img width="381" alt="image" src="https://user-images.githubusercontent.com/106216912/221392807-528cf812-6816-4c74-8b20-5e6435479547.png">

- **지역(locality)** : 집중적으로 함께 참조되는 페이지들의 집합
    - **실행 중인 프로그램은 여러 개의 지역으로 구성되어 있고, 이 지역들은 서로 겹쳐질 수도 있다.**

</br>

> **어떤 프로세스에 현재 지역성을 포함하기에 충분한 프레임을 제공해 준다고 가정한다.**

- 그 프로세스는 현재 지역의 모든 페이지가 메모리로 올라올 때까지는 페이지 폴트를 발생시키지만, 그 후에는 지역이 변경되기 전까지는 페이지 폴트를 발생시키지 않는다.
    - 필요로 하는 지역성의 크기보다 적은 프레임을 할당하게 되면, 그 프로세스는 접근해야 하는 모든 페이지를 메모리에 유지할 수 없기 때문에 지속해서 페이지 폴트를 발생시킨다.
...
- 이 모델은 개념적으로 **창(작업 집합 : window)** 를 가지고 있는 것과 같다.

</br>

<img width="478" alt="image" src="https://user-images.githubusercontent.com/106216912/221392825-ceefb758-d08d-4fa5-8e22-7e881a8fd332.png">

- 기본 아이디어는 **최근 Δ만큼의 페이지 참조를 관찰하겠다**는 것이다.
    - 한 프로세스가 최근 Δ번 페이지를 참조했다면, 그 안에 들어있는 서로 다른 페이지들의 집합을 **작업 집합**이라고 부른다.
    - 페이지가 활발하게 사용되었으면 작업 집합에 포함되고, 더 사용되지 않게 되면 마지막 참조로부터 Δ만큼의 페이지 참조 후에는 작업 집합에서 제외된다.

</br>

> **작업 집합의 정확도는 Δ의 선택에 따라 좌우한다.**

- Δ값이 너무 작으면 전체 지역을 포함하지 못할 것이고, Δ값이 너무 크면 여러 지역성을 과도하게 수용할 것이다.

</br>

> **이 모델의 가장 중요한 요소는 이 집합의 크기이다.**

- 각 프로세스가 `WSSi` 만한 크기의 공간을 작업 집합의 크기로 요구한다면, 모든 프로세스 전체의 요구량 `D` 는 다음과 같다.

<img width="119" alt="image" src="https://user-images.githubusercontent.com/106216912/221392851-5da3a979-2018-45d3-8c2f-653c88e2523d.png">

</br>

- 각 프로세스는 작업 집합 내의 페이지를 활발하게 사용한다.
    - 즉, 프로세스 `i` 는 `WSSi` 만큼의 프레임을 요구한다.
    - 그런데 요구량 `D` 가 시스템이 보유한 총 메모리 크기 `m` 보다 커지면, 어떤 프로세스는 지역을 위한 충분한 프레임을 메모리에 가질 수 없게 되기 때문에 스래싱을 유발한다.

</br>

> **Δ값을 선택하고 난 후, 이 모델의 사용법**

- 운영체제는 각 프로세스의 작업 집합을 감시하면서 각 프로세스에 작업 집합 크기에 맞는 충분한 프레임을 할당한다.
    - 이렇게 할당하고도 여분의 놀고 있는 프레임이 있다면 또 새로운 프로세스를 시작시킬 수 있다.
    - 하지만 실행 중인 프로세스의 수가 너무 많아져서 `D` 가 `m` 보다 커지게 되면, 운영체제는 프로세스를 하나 선택해서 그 프로세스의 페이지를 빼앗고, 연기시키고, 그 프레임들은 다른 프로세스에 주게 된다.
...
<img width="438" alt="image" src="https://user-images.githubusercontent.com/106216912/221392882-e275de19-c41a-467a-96f4-4f2594270eda.png">

- 스래싱 = 페이지 폴트율이 높다.
    - **페이지 폴트율이 너무 높음 = 그 프로세스가 더 많은 프레임이 필요함**
    - **페이지 폴트율이 너무 낮음 = 프로세스가 너무 많은 프레임을 갖고 있음**

</br>

→ 따라서, **페이지 폴트율의 상한과 하한을 정해 놓고, 만약 페이지 폴트율이 상한을 넘으면 그 프로세스에 프레임을 더 할당해주고, 하한보다 낮아지면 그 프로세스의 프레임 수를 줄인다.**

</br>

### 현재 관행

→ ***현실적으로 스래싱과 그에 따른 스와핑은 달가워할 수 없을 만큼 성능에 영향을 미친다.***

- 스래싱과 스와핑을 피하고자 컴퓨터 시스템을 구현하는 현재 가장 최선책은, 가능한 한 충분한 물리 메모리를 장착하는 것이다!
...
- **여기서는 수정된 프레임을 스왑 공간으로 페이징 아웃하지 않고, 여러 프레임을 하나의 프레임으로 압축한다.**
    - 이러면 **시스템이 페이지 스와핑에 의존하지 않고도 메모리 사용량을 줄일 수 있게 한다.**

</br>

<img width="430" alt="image" src="https://user-images.githubusercontent.com/106216912/221393492-c34f37da-1c13-4750-b5b0-091583c402e3.png">

- 위의 가용 프레임 리스트에서는 6개의 프레임이 있다.
    - 이 가용 프레임 수는 페이지 교체를 촉발하는 임계값보다 적다고 가정한다.

</br>

- 교체 알고리즘은 4개의 프레임을 선택하여 가용 프레임 리스트에 넣는다.
    - 우선 수정된 프레임 리스트에 넣고, 수정된 프레임 리스트는 스왑 공간에 기록되고 프레임들은 가용 프레임 리스트에 넣어진다.

</br>

> **다른 전략은 여러 프레임을 압축하고, 압축된 버전을 하나의 페이지 프레임에 저장하는 것이다.**

<img width="483" alt="image" src="https://user-images.githubusercontent.com/106216912/221393513-67b29c2d-3c8a-48f9-bc17-7c42229bd182.png">

- 프레임 7은 가용 프레임 리스트에서 제거된다.
    - 프레임 15, 3 및 35는 압축되어 프레임 7에 저장되고, 압축된 프레임 리스트에 저장된다.
    - 프레임 15, 3 및 35는 이제 가용 프레임 리스트로 이동될 수 있다.
    - 세 개의 압축된 프레임 중 하나가 나중에 참조되면 페이지 폴트가 발생하고, 압축된 프레임이 압축 해제되어 세 개의 페이지 15, 3 및 35가 메모리에 복원된다.

</br>

> **모바일 시스템은 일반적으로 표준 스와핑 또는 페이지 스와핑을 지원하지 않는다.**

→ ***따라서, 메모리 압축은 Android 및 iOS를 포함한 대부분의 모바일 운영체제의 메모리 관리 전략의 핵심 부분이다.***

- Windows 10과 macOS는 모두 메모리 압축을 지원한다.

</br>

> **메모리 압축을 위해서는 압축된 페이지를 유지하기 위해 사용 가능한 프레임을 할당해야 하지만, 압축 알고리즘으로 얻은 메모리 감소에 따라 상당한 메모리를 절약할 수 있다.**
> 
- **압축률** → 데이터 압축의 유형과 마찬가지로, 압축 알고리즘의 속도와 얻을 수 있는 감소량 사이에 경합이 있다.
    - 일반적으로 더 느리고 계산 비용이 많이 드는 알고리즘을 사용하면, 더 높은 압축률을 얻을 수 있다.
...
1. **커널은 다양한 크기의 자료구조를 위해 메모리를 할당받는다.**
    ...
    - 따라서, 커널은 메모리를 조심스럽게 사용해야 하고 단편화에 의한 낭비를 최소화하고자 한다.

</br>

2. **사용자 모드 프로세스에 할당되는 페이지들은 물리 메모리상에서 굳이 연속된 것일 필요는 없다.**
    - 하지만, 가상 메모리 인터페이스를 통하지 않고 물리 메모리에 직접 접근하는 특정 하드웨어 장치는 물리적으로 연속적인 메모리가 있어야 하는 경우가 있다.
...
- **메모리는 이 세그먼트로부터 2의 거듭제곱 할당기에 의해 2의 거듭제곱 단위로 할당된다(4KB, 8KB, 16KB).**
    - 2의 거듭제곱 크기가 아닌 메모리 요구는 가장 가까운 2의 거듭제곱 크기로 올림 된다.

</br>

<img width="379" alt="image" src="https://user-images.githubusercontent.com/106216912/221394139-4f8e05ad-7065-43b7-ab32-7b3d86c7ef60.png">

- 메모리 세그먼트의 크기는 초기에 256KB라고 가정하고, 커널이 21KB의 메모리를 요구했다고 가정한다.
    - 세그먼트는 128KB 크기의 두 개의 버디(buddies)로 나뉜다.
        - 그 중 하나는 다시 두 개의 64KB 크기의 버디로 나누어진다.
        - 하지만, 21KB의 가장 가까운 2의 거듭제곱은 32KB이므로 다시 두 개의 32KB 버디로 나뉜다.

</br>

> **버디 시스템의 이점**

→ ***합병이라고 부르는 과정을 통해 서로 인접한 버디들이 손쉽게 하나의 큰 세그먼트로 합쳐질 수 있다.***

</br>

> **버디 시스템의 단점**

→ ***가장 가까운 2의 거듭제곱으로의 올림이, 할당된 세그먼트 내의 단편화를 가져온다.***

</br>

### 슬랩 할당

→ ***두 번째 커널 메모리 할당 정책은 슬랩 할당으로, 슬랩(slab)은 하나 또는 그 이상의 연속된 페이지들로 구성된다.***

<img width="435" alt="image" src="https://user-images.githubusercontent.com/106216912/221394167-1f7ae17d-33ad-4ed6-9823-0b0d36ab4865.png">

- 캐시(cache)는 하나 혹은 그 이상의 슬랩들로 구성된다.
    - 각 커널 자료구조마다 하나의 캐시가 존재한다.
    - 캐시는 커널 자료구조의 각 인스턴스에 해당하는 객체들로 채워져 있다.
    - 각 객체는 개별 캐시에 저장된다.
...
- 슬랩은 다음과 같은 세 가지 상태 중 한 상태에 있게 된다.
    1. `Full` : 슬랩 내의 모든 객체가 `used` 로 표시됨
    2. `Empty` : 슬랩 내의 모든 객체가 `free` 로 표시됨
    3. `Partial` : `used` , `free` 객체가 섞여 있음

</br>

→ 슬랩 할당기는 먼저 `partial` 슬랩의 `free` 객체를 이용해 요청을 처리하려고 시도한다.

- `partial` 슬랩이 없으면 `empty` 슬랩으로부터 `free` 객체를 할당한다.
    - `empty` 슬랩도 없는 경우에는 새로운 슬랩이 연속된 물리 메모리에서 할당되어 캐시에 주어지고, 객체는 이 슬랩에 할당된다.
...
2. 메모리 요청이 빠르게 처리된다.
    - 슬랩 할당기는 할당과 해제가 빈번한 자료구조 객체를 관리하는 데 특히 효율적이다.
