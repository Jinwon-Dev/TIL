...
→ ***정보가 컴퓨터 시스템에 저장되어 있을 때, 물리적인 손상으로부터 그리고 부적절한 접근으로부터 안전하길 원한다.***

- 신뢰성은 일반적으로 파일의 복사본에 의해 제공된다.
    - 많은 컴퓨터는 파일 시스템이 사고로 파괴될 경우에 대비하여 복사본을 유지하기 위해 규칙적인 간격으로 디스크 파일을 테이프에 자동 복사하는 프로그램이 있다.

</br>

> **보호는 여러 가지 방법으로 제공될 수 있다.**
> 
- 최신 운영체제를 실행하는 랩톱 시스템의 경우 시스템에 접근하기 위해서는 사용자 이름 및 암호 인증을 요구하여 보호를 제공한다.
    - 또한 보조저장장치를 암호화하여 랩톱을 열고 드라이브를 훔쳐 간 사람도 데이터에 액세스하는 데 어려움을 겪게 만든다.
    - 방화벽으로 네트워크 액세스를 보호하여 사용 중일 때 네트워크 연결을 통해 침입하기 어렵게 만들어 보호를 제공할 수 있다.
...
- 신원에 기반한 접근을 구현하는 가장 일반적인 방법은 각 파일과 디렉터리에 접근 제어 리스트(ACL)를 연관해 두는 것이다.
    ...
    - 그리고 그 사용자가 특정 파일에 대한 접근을 요청할 때, 운영체제는 그 파일의 접근 리스트를 검사하여 허용 여부를 결정한다.

</br>

> **이 접근 방법은 복잡한 접근 방법을 가능하게 하는 장점이 있다.**
> 
- 이러한 기법은 두 가치의 원치 않는 결과를 가진다.
    - 그러한 리스트를 구축하는 일은 지겹고 별 소득 없는 일이고, 특히 우리는 시스템 사용자의 리스트를 미리 알 수 없다.
    - 고정 크기였던 디렉터리 항목이 가변 크기가 되어 버려 결과적으로 더욱 복잡한 공간 관리 문제가 발생한다.

</br>

> **이러한 문제점들은 접근 리스트를 간소화함으로써 해결될 수 있다.**
> 
- 접근 리스트의 길이를 간결하게 하기 위해서 많은 시스템들은 모든 사용자를 세 가지 부류로 분류한다.
...
→ ***파일 보호를 위해 사용될 수 있는 또 다른 방법은 각 파일에 접근할 때, 암호를 요구하는 것이다.***

- 만일 암호가 무작위로 선택되고 자주 변경된다면 이 기법은 파일 접근을 암호로 알고 있는 사람으로 제한하는 데 효과적일 것이다.

</br>

> **하지만, 이러한 방법은 몇 가지 단점이 있다.**
> 
...
→ ***파일에 액세스하는 또 다른 방법이 있으며, 매우 일반적으로 사용된다.***

- 디스크 입출력을 메모리 참조 방식으로 대신할 수도 있다.
    - 이러한 **메모리 사상**이라고 불리는 접근 방식은 프로세스의 가상 주소 공간 중 일부를 관련된 파일에 할애하는 것을 말한다.
...
- 첫 번째 접근은 일반적인 요구 페이징 과정에 따라 페이지 폴트가 발생한다.
    - 이 때 그 파일 내용 중 페이지 크기만큼의 해당 부분이 파일 시스템으로부터 메모리 페이지로 읽혀 들어오게 된다.
    - 그 이후의 파일 read/write는 일반적인 메모리 액세스와 같이 처리된다.

</br>

> **일부 운영체제는 특정 시스템 콜을 통해서만 메모리 사상 파일 처리를 해주고, 일반적으로는 표준 파일 관련 시스템 콜을 통해 파일에 접근하도록 한다.**
> 
- 반면 어떤 운영체제는 메모리 사상 파일을 표준으로 삼기도 한다.

</br>

> **여러 프로세스가 데이터 공유를 위해 이러한 파일을 공유하는 수도 있다.**
> 
<img width="388" alt="image" src="https://user-images.githubusercontent.com/106216912/225968801-36e2f0ad-b9d2-4f84-aa53-b94e80d2620a.png">

- 이러한 경우 한 프로세스가 공유 중인 메모리 사상 파일에 write를 하면 그 write는 즉시 다른 모든 프로세스도 볼 수 있게 된다.

</br>

> **사실 공유 메모리를 메모리 사상 파일을 이용하여 구현하는 것은 매우 자주 있는 일이다.**
> 
<img width="430" alt="image" src="https://user-images.githubusercontent.com/106216912/225968885-7f715a3b-ebde-4004-80a9-5712db389826.png">

- 이러한 방식에서는 동일한 파일을 프로세스들의 가상 주소 공간에 메모리 사상하게 함으로써 프로세스들은 공유 메모리를 사용하여 통신할 수 있다.
    - 메모리 사상된 파일은 통신하는 프로세스들 사이의 공유 메모리 영역으로서 동작한다.